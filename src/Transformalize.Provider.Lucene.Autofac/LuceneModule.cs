...
namespace Transformalize.Providers.Lucene.Autofac {
    ...
    public class LuceneModule : Module {
        ...
        protected override void Load(ContainerBuilder builder) {
                        ...
                        foreach (var analyzer in process.SearchTypes.Where(st => st.Analyzer != string.Empty && st.Analyzer != DefaultAnalyzer).Select(st => st.Analyzer).Distinct()) {
                            switch (analyzer) {
                                ...
                                case "whitespace":
                                    ...
                                    break;
                                case "standard":
                                    builder.Register<Analyzer>(ctx => new global::Lucene.Net.Analysis.Standard.StandardAnalyzer(global::Lucene.Net.Util.Version.LUCENE_30)).Named<Analyzer>(analyzer);
                                    break;
                                default:
            ...
            // entity input
            foreach (var entity in process.Entities.Where(e => process.Connections.First(c => c.Name == e.Connection).Provider == "lucene")) {
            ...
            // entity output
            if (process.Output().Provider == "lucene") {
